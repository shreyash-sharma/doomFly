...
class Game:
    def __init__(self,seed=41027,scenario='defend_the_center',spectator=False,observer_engine=None,show_hud=False):
        ...
        # Presentation-only option. Experiment defaults preserve the scenario
        # HUD setting; replay renderers may expose Doom's native status bar.
        self.game.set_render_hud(bool(show_hud))
        ...
        self.game.set_available_buttons([vzd.Button.TURN_LEFT_RIGHT_DELTA,
            vzd.Button.MOVE_FORWARD_BACKWARD_DELTA,vzd.Button.MOVE_LEFT_RIGHT_DELTA,
            vzd.Button.ATTACK])
        ...
        self.game.set_button_max_value(vzd.Button.MOVE_LEFT_RIGHT_DELTA,20)
        ...
        # Observer/reinforcement-only combat counters. They are never supplied
        # to a neural action decoder.
    ...
    def act(self,action):
        ...
        reward=self.game.make_action([action['turn'],action['forward'],
            float(action.get('strafe',0.0)),int(action['attack'])],1)
        self.tick+=1
        return float(reward)
    def spectator(self):
    ...
    def observation(self):
        ...
        result={'episode':self.episode,'tick':self.tick,'finished':g.is_episode_finished(),
          ...
          'damage_inflicted':int(g.get_game_variable(vzd.GameVariable.DAMAGECOUNT)),
          'damage_taken_counter':int(g.get_game_variable(vzd.GameVariable.DAMAGE_TAKEN)),
